...
function buildResolveRepoDirLines() {
  return [
    'resolve_repo_dir() {',
    '  local input_path="$1"',
    '  local candidate=""',
...
function buildNpmVerifyScript() {
  return [
    ...
    '',
    'ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"',
    'REPO_DIR_INPUT="${1:-repo}"',
    ...buildResolveRepoDirLines(),
    'REPO_DIR="$(resolve_repo_dir "$REPO_DIR_INPUT")"',
    '',
    'if [[ ! -d "$REPO_DIR" ]]; then',
    '  echo "Repository directory does not exist: $REPO_DIR_INPUT" >&2',
    '  exit 1',
    'fi',
    '',
    ...
    '    echo "Typecheck failed. See $RESULT_DIR/typecheck.log" >&2',
    '    write_results',
    '    exit 1',
    '  fi',
    'else',
    '  echo "Skipping typecheck: no npm script named typecheck"',
    'fi',
    '',
